...
&lt;http://lodpaddle.univ-nantes.fr/bibliotheque_mediatheque&gt; rdf:type void:Dataset ;
	foaf:homepage &lt;http://data.nantes.fr/donnees/detail/bibliotheques-et-mediatheques-en-loire-atlantique/&gt;;
	dcterms:title "Bibliothèques et Médiathèques en Loire-Atlantique"^^xsd:string ;
	dcterms:description "Bibliothèques et Médiathèques en Loire-Atlantique"^^xsd:string ;
...
<xsl:text>&#009;rdf:type&#009; pdll:bibliotheque_mediatheque;
</xsl:text>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/bibliotheque_mediatheque/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
