...
<xsl:text>&#009;rdf:type&#009; pdll:cinema;
</xsl:text>
	<xsl:apply-templates select="COMMUNE"/>
	<xsl:apply-templates select="_l"/>
	<xsl:apply-templates select="SITE_INTERNET"/>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.com/cinema/',translate(translate(translate(translate(translate(translate(translate(./text(),'&quot;',' '),'«',' '),'&gt;',' '),'&lt;',' '),'  ',' '),' ','_'),'.','_')),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
