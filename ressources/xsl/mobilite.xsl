...
&lt;http://lodpaddle.univ-nantes.fr//mobilite&gt; rdf:type void:Dataset ;
	foaf:homepage &lt;http://data.nantes.fr/donnees/detail/localisation-des-equipements-publics-relevant-du-theme-mobilite-de-nantes-metropole-1/&gt;;
	dcterms:title "Localisation des équipements publics relevant du thème 'Mobilité' de Nantes Métropole"^^xsd:string ;
	dcterms:description "Localisation des équipements publics relevant du thème 'Mobilité' de Nantes Métropole"^^xsd:string ;
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/mobilite/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
