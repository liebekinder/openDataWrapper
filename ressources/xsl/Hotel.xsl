...
&lt;http://lodpaddle.univ-nantes.fr/Hotel&gt; rdf:type void:Dataset ;
	foaf:homepage &lt;http://data.nantes.fr/donnees/detail/hotels-en-loire-atlantique/&gt;;
	dcterms:title "Hôtels en Loire-Atlantique"^^xsd:string ;
	dcterms:description "Hôtels en Loire-Atlantique"^^xsd:string ;
...
<xsl:choose><xsl:when test=". = 'null'">&#009;pdll:numberOfRoom&#009; "undefined"^^xsd:string ;
</xsl:when>
<xsl:otherwise>&#009;pdll:numberOfRoom&#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
...
<xsl:otherwise><xsl:value-of select="concat(concat('&lt;http://lodpaddle.univ-nantes.fr/Hotel/',encode-for-uri(.)),'&gt;')"/>&#009; foaf:name &#009; "<xsl:value-of select="translate(., '&quot;','')"/>"^^xsd:string ;
</xsl:otherwise></xsl:choose></xsl:template>
...
<xsl:template match="CAPACITE_PERS"><xsl:choose><xsl:when test=". = 'null'">&#009;dbpprop:seatingCapacity&#009; "-120404040"^^xsd:int ;
</xsl:when>
